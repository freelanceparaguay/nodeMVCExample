...
app.get('/',function(req,res){
    console.log("en / req.session.username="+req.session.username);    
...
app.get('/login',function(req,res){
    console.log("en /login req.session.username="+req.session.username);
    if(req.session.username)
        res.redirect('/dashboard');
...
app.post('/login',function(req,res){
   console.log("process.check----> antes de la funcion");
   process.check(req,hash,function(status,msg){        
        console.log("cuerpo llamada process.check Status : %s , message : %s ",status ,msg);
        if(status){
            
            if(req.session.login_error)
                delete req.session.login_error;
            console.log("POST /login req.session.username="+req.session.username);
            res.redirect('/dashboard');
        }
        else{       
            res.redirect('/login');
   ...
   console.log("process.check----> despues de la funcion");   
});
