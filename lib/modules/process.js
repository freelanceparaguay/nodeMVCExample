...
exports.getRowServer = function(req,params, fn){
    ...
    req.getConnection(function(err,connection){
        ...
        if(params.qsearch !='' && params.filter_by=='')
            if_search +=" AND moduleName LIKE '%"+params.qsearch+"%' ";
...
exports.saveRowServer = function (req,fn) {
    ...
    req.getConnection(function (err, connection) {
    	if(temp.idModal==''){
    		var insert = {
    				id:'0',
           		 	urlModule:temp.field1Modal,
           		 	moduleName: temp.field2Modal
    				};
    		connection.query("INSERT INTO "+MODULE_TABLE+" set ? ",insert, function(err, rows){
    			if (err){
    				return fn(false,err);
    			}
    			return fn(true," module created");                  
            });//connection.query
